...
int Cave::count_live_neighbors(std::vector<std::vector<int>> vv, int i, int j) {
  int count = 0;
  if ((i - 1) >= 0 && (i - 1) <= vv.size() - 1 && (j - 1) >= 0 &&
      (j - 1) <= vv.size() - 1 && (vv[i - 1][j - 1] == 1))
    count++;
  if (i >= 0 && i <= vv.size() - 1 && (j - 1) >= 0 &&
      (j - 1) <= vv.size() - 1 && (vv[i][j - 1] == 1))
    count++;
  if ((i + 1) >= 0 && (i + 1) <= vv.size() - 1 && (j - 1) >= 0 &&
      (j - 1) <= vv.size() - 1 && (vv[i + 1][j - 1] == 1))
    count++;
  if ((i - 1) >= 0 && (i - 1) <= vv.size() - 1 && j >= 0 &&
      j <= vv.size() - 1 && (vv[i - 1][j] == 1))
    count++;
  if ((i + 1) >= 0 && (i + 1) <= vv.size() - 1 && j >= 0 &&
      j <= vv.size() - 1 && (vv[i + 1][j] == 1))
    count++;
...
void Cave::next_generation(std::vector<std::vector<int>> &v1,
